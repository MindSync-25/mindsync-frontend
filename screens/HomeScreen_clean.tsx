...
const HomeScreen: React.FC = () => {
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    searchBar: {
      flexDirection: 'row',
      alignItems: 'center',
      backgroundColor: theme === 'light' ? '#f0f0f0' : '#232323',
      borderRadius: 12,
  ...
  // === Real task data from TaskContext ===
  // Get all incomplete tasks for to-do list
  const incompleteTasks = state.tasks.filter(task => task.status !== 'completed');
  const tasksDue = incompleteTasks.length;
  
  // Show first 2 tasks, and count remaining
  const displayTasks = incompleteTasks.slice(0, 2);
  const remainingTasksCount = incompleteTasks.length - 2;
  
  // === Sample / placeholder data for other widgets ===
    ...
    <View style={{ flex: 1, backgroundColor: theme === 'light' ? '#fff' : '#000' }}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.centeredContent}>
          ...
          {/* Search Bar */}
          <View style={[dynamicStyles.searchBar, styles.maxCardWidth]}>
          ...
          <View style={styles.maxCardWidth}>
            <View style={styles.grid}>
              {/* To-Do List */}
              <TouchableOpacity style={[dynamicStyles.card, styles.gridItemLeft]} onPress={() => navigation.navigate('TaskManagement')} activeOpacity={0.8}>
                <Text style={dynamicStyles.cardTitle}>To-Do List</Text>
                {displayTasks.length > 0 ? (
                  <>
                    {displayTasks.map((task, i) => (
                      <View key={task.id} style={styles.listItem}>
                        <MaterialCommunityIcons name="checkbox-blank-outline" size={18} color={theme === 'light' ? '#666' : '#f5f5f5'} />
                        <Text style={dynamicStyles.listItemText}>{task.title}</Text>
                      </View>
                    ))}
                    {remainingTasksCount > 0 && (
                      <Text style={[dynamicStyles.listItemText, { fontStyle: 'italic', marginTop: 8 }]}>
                        +{remainingTasksCount} more tasks
                      </Text>
                    )}
                  </>
                ) : (
                  <Text style={[dynamicStyles.listItemText, { fontStyle: 'italic' }]}>No tasks</Text>
                )}
              </TouchableOpacity>
              ...
              {/* Reminders */}
              <TouchableOpacity style={[dynamicStyles.card, styles.gridItemLeft]} onPress={() => navigation.navigate('ComingSoon')} activeOpacity={0.8}>
                <Text style={dynamicStyles.cardTitle}>Reminders</Text>
